...
class JourneyDaoTest {
    ...
    @Test
    fun testInsertJourney() = runBlocking {
        val journey = EntityJourney("1", "one", "desc", 100L, 100L)
        journeyDao.insertJourney(journey)
        val retrievedJourney = journeyDao.getJourneyById("1")
        assert(retrievedJourney!!.title == journey.title)
    }



    @Test
    fun testInsertPlaceTojourney() = runBlocking {
        val journey = EntityJourney("1", "one", "desc", 100L, 100L)
        journeyDao.insertJourney(journey)
